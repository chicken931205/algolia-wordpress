...
if (!class_exists('Knowledge_Base_Embed_Settings')) {
	...
	class Knowledge_Base_Embed_Settings {
		...
		/*
		* Test if embed functionality is enabled
		*/
		function ht_kb_display_embed_settings_section(){
			//don't run if display disabled
			$ht_kb_license_valid = $this->kb_settings_is_license_status_valid();
			//can be overriden manually, or by theme
			return apply_filters( 'ht_kb_display_embed_settings_section', $ht_kb_license_valid );
		}

		function ht_kb_embed_settings_tab(){
			if(apply_filters('hkb_add_embed_settings_section', true)): ?>
				<a href="#embed-section" id="embed-section-link" data-section="embed"><?php _e('Help Assistant', 'ht-knowledge-base'); ?></a>
			<?php endif;
		}
		...
		function ht_kb_fe_embed_settings_section_display(){
			if( apply_filters('hkb_add_embed_settings_section', true) ): ?>
				<?php
					...
					$display_embed_settings_section = $this->ht_kb_display_embed_settings_section();
					$suppress_license_disabled_warning = apply_filters( 'ht_kb_embed_suppress_license_warning', false );

					if( $display_embed_settings_section ){
					...
					<?php 
						if( $display_embed_settings_section ){
							...
							if( !$suppress_license_disabled_warning ){
								$this->ht_kb_settings_embed_license_disabled_message();
							} else {
								//do nothing?
							}
						}
						do_settings_sections('ht_kb_settings_embed_section');						
					?>    
				</div>
			<?php endif; 
		}  
		...
		function ht_kb_fe_embed_settings_validate($output, $input){
			...
			if( ! isset( $_POST['_ht_kb_embed_settings_security'] ) ||
				! wp_verify_nonce( $_POST['_ht_kb_embed_settings_security'], 'ht_kb_settings_embed_settings_security' ) 
			) {
				//keep previous settings 
				$output['kb-fe-embed-server'] = isset($ht_knowledge_base_settings['kb-fe-embed-server']) ? $ht_knowledge_base_settings['kb-fe-embed-server'] : 0;
				$output['kb-fe-embed-enable'] = isset($ht_knowledge_base_settings['kb-fe-embed-enable']) ? $ht_knowledge_base_settings['kb-fe-embed-enable'] : 0;
				$output['kb-fe-embed-target-url'] = isset($ht_knowledge_base_settings['kb-fe-embed-target-url']) ? $ht_knowledge_base_settings['kb-fe-embed-target-url'] : '';
				$output['kb-fe-embed-customize-color'] = isset($ht_knowledge_base_settings['kb-fe-embed-customize-color']) ?  $ht_knowledge_base_settings['kb-fe-embed-customize-color'] : '#2d9bf3';
		...
		function ht_kb_fe_embed_settings_section_fields(){
			...
			if( $display_embed_settings_section ){
				...
				add_settings_section('ht_knowledge_base_embed_settings', __('Help Assistant Options', 'ht-knowledge-base'), array($this, 'ht_kb_settings_embed_section_description'), 'ht_kb_settings_embed_section'); 
				...
				add_settings_field('kb-fe-embed-server', __('Enable Help Assistant', 'ht-knowledge-base'), array($this, 'integrations_kb_fe_embed_server_option_render'), 'ht_kb_settings_embed_section', 'ht_knowledge_base_embed_settings');

				add_settings_field('kb-fe-embed-enable', __('Show Help Assistant on this Site', 'ht-knowledge-base'), array($this, 'integrations_kb_fe_embed_enable_option_render'), 'ht_kb_settings_embed_section', 'ht_knowledge_base_embed_settings');
				...
				add_settings_field('kb-fe-embed-snippet-generate-dummy', __('Add Help Assistant to External Site', 'ht-knowledge-base'), array($this, 'integrations_kb_fe_embed_snippet_generate_dummy_option_render'), 'ht_kb_settings_embed_section', 'ht_knowledge_base_embed_settings');

				add_settings_field('kb-fe-embed-customize-dummy', __('Customize Help Assistant', 'ht-knowledge-base'), array($this, 'integrations_kb_fe_embed_section_customize_dummy_option_render'), 'ht_kb_settings_embed_section', 'ht_knowledge_base_embed_settings');
		...
		function ht_kb_settings_embed_section_description(){
			...
			<?php
				...
				wp_nonce_field( 'ht_kb_settings_embed_settings_security', '_ht_kb_embed_settings_security' );
		}

		function ht_kb_settings_embed_license_disabled_message(){
			$licenses_and_updates_url = apply_filters( 'ht_kb_display_embed_settings_license_entry_link', admin_url( 'edit.php?post_type=ht_kb&page=ht_knowledge_base_settings_page#license-section' ) );

			$license_entry_message = sprintf( __('You must activate your license to enable the embed functionality, please enter and activate your license from the <a href="%s">Licenses and Updates section</a> now.', 'ht-knowledge-base' ), $licenses_and_updates_url );

			echo apply_filters( 'ht_kb_settings_embed_license_disabled_message', $license_entry_message );			
		}

		function integrations_kb_fe_embed_section_title_dummy_option_render(){
			?>
				<div><?php _e('Use the Knowledge Base Help Assistant on this or another site', 'ht-knowledge-base'); ?></div>              
			<?php            
		}
		...
		function integrations_kb_fe_embed_section_customize_color_option_render(){
			...
			if(!apply_filters('integrations_kb_fe_embed_color_option_render', true)){
			 ...
			 ?>
				...
				<span class="hkb_setting_desc"><?php _e('Color of Help Assistant', 'ht-knowledge-base'); ?></span>             
			<?php            
		}
		...
		*/
		...
		function kb_fe_embed_get_server_enabled( $bool ){
			...
			//requires license active, hook later to kb_fe_embed_get_server_enabled to override
			//nb. in this implementation we use a filter in kb_settings_is_license_status_valid to override the valid boolean
			$kb_fe_embed_get_server_enabled = $this->kb_settings_is_license_status_valid() ? $kb_fe_embed_get_server_enabled : false;
		...
		function kb_fe_embed_get_embed_enabled( $bool ){
			...
			//requires license active, hook later to kb_fe_embed_get_embed_enabled to override
			$kb_fe_embed_get_embed_enabled = $this->kb_settings_is_license_status_valid() ? $kb_fe_embed_get_embed_enabled : false;
