    ...
    class HT_Knowledge_Base_Data_Tools {
        ...
        */
        function hkb_data_tools_action_buttons(){
            if( current_user_can( apply_filters( 'ht_kb_data_tools_capability', 'manage_options' ) ) ){
                ...
                $purge_hkb_recalculate_article_reading_time_data_url = admin_url('edit.php?post_type=ht_kb&page=ht_knowledge_base_debug_info' . '&purge_hkb_data=recalculate_article_reading_time');
                $purge_hkb_recalculate_article_reading_time_data_sec_url = wp_nonce_url( $purge_hkb_recalculate_article_reading_time_data_url, 'hkb_purge_data_action_' . 'recalculate_article_reading_time', 'security_token' );
                ...
                ?>
                    ...
                    <div class="hkb-data-tool-actions">
                        ...
                        <br/><br/>
                        <a class="button hkb-tools_button" href="<?php echo $purge_hkb_recalculate_article_reading_time_data_sec_url; ?>" data-confirm-message="<?php esc_attr_e('Are you sure you want to recalculate reading time for all articles now?', 'ht-knowledge-base'); ?>"><?php _e('Recalculate Reading Time', 'ht-knowledge-base'); ?></a
                    </div>
        ...
        */
        ...
        function hkb_data_tools_action_handler(){
            ...
            $data_tools_action = array_key_exists('purge_hkb_data', $_GET) ? $_GET['purge_hkb_data'] : false;

            if( !$data_tools_action ){
                return;
            }

            //set allowable actions
            $allowable_actions = array('all','search','voting','exits','visits','recalc_usefulness','recalc_visits', 'delete_votes_for_deleted_articles', 'recalculate_article_reading_time' );

            $data_tools_action = in_array($data_tools_action, $allowable_actions) ? $data_tools_action : die('Not an allowed knowledge base data tool action');

            //early exit
            if( empty($data_tools_action) ){
                return;
            }
            ...
            if( current_user_can( apply_filters('ht_kb_data_tools_capability', 'manage_options') ) ){
                ...
                //check security
                if (!isset($_GET['security_token']) || !wp_verify_nonce($_GET['security_token'], 'hkb_purge_data_action_' . $data_tools_action)) {
                    die('Security Check');
                }
                switch ($data_tools_action) {
                    case 'all':
                        ...
                        $this->hkb_data_tools_recalc_reading_time();
                        break;
                    case 'search':
                        $this->hkb_data_tools_clear_all_search_data();
                        break;
                    ...
                    case 'delete_votes_for_deleted_articles':
                        ...
                        break;
                    case 'recalculate_article_reading_time':
                        $this->hkb_data_tools_recalc_reading_time();
                        break;
                    default:
                        break;
        ...
        function hkb_data_tools_recalc_reading_time(){
            try {
                if(class_exists('HT_Knowledge_Base_Reading_Time')){
                    ...
                    //loop and update reading time
                    foreach ( $ht_kb_posts as $post ) {
                        //update
                       apply_filters('ht_kb_set_post_reading_time', $post->ID);           
                    }
                    $this->admin_notices[] = array('state'=>'success', 'text'=>__('Article reading time successfully updated', 'ht-knowledge-base'));
                }  else {
                    $this->admin_notices[] = array('state'=>'info', 'text'=>__('Reading time functionality not found', 'ht-knowledge-base'));
                }
            } catch (Exception $e) {
                $this->admin_notices[] = array('state'=>'error', 'text'=>__('Something went wrong updating article reading time', 'ht-knowledge-base'));
            }
        }
