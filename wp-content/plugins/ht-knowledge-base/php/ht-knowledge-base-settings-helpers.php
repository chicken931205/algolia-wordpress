...
if(!function_exists('ht_kb_get_kb_archive_post_name')){
    /**
    * fetch knowledge base archive page post name
    * Filterable - ht_kb_get_kb_archive_post_name
    * @since 3.4
    * @return (String) $post_name
    */
    function ht_kb_get_kb_archive_post_name( $kb_key = 'default' ){
        global $ht_knowledge_base_settings;
        //default 
        $post_name = false;
        $post = get_post( ht_kb_get_kb_archive_page_id( $kb_key ) );
        if ( is_a( $post, 'WP_Post') ){
            $post_name = $post->post_name;       
        }

        return apply_filters( 'ht_kb_get_kb_archive_post_name', $post_name, $kb_key );
    }
}
...
if(!function_exists('ht_kb_get_registered_knowledge_base_archive_post_names')){
    /**
    * fetch all knowledge base archive page post name
    * Filterable - ht_kb_get_registered_knowledge_base_archive_post_names
    * @since 3.4
    * @return (Array) string array $post_names
    */
    function ht_kb_get_registered_knowledge_base_archive_post_names(){
        global $ht_knowledge_base_settings;
        //default 
        $post_names = array();
        ...
        foreach ($knowledge_bases as $index => $key) {
                //kb archive page id
                $page_name = ht_kb_get_kb_archive_post_name( $key );
                if($page_name){
                    $post_names[] = $page_name;
                } 
        }

        return apply_filters( 'ht_kb_get_registered_knowledge_base_archive_post_names', $post_names );
    }
}
