...
function ht_knowall_setup_customizer_options_upgrade() {
	...
	if ( $ht_setting__kbarchivehideemptycats === false ) {
		//possibly no longer required after changes to boolean option handling in kirki?
...
/**
* Don't show welcome screen - use new welcome setup
*/
add_filter( 'show_ht_kb_welcome_on_activation', '__return_false' );

/**
* Don't show run HKB setup assistant prompt - this is handled by KA welcome setup
*/
add_filter( 'ht_kb_maybe_run_setup_assistant_notice_display', '__return_false' );
...
function ht_is_widget_in_sidebar( $widget_id, $sidebar_id ) {
