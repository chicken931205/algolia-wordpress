		...
		*/
		public function widget( $args, $instance ) {
				...
				if ( in_array($instance['sort_by'], $valid_sort_orders) ) {
					...
					//set the sort order as ascending if set, unless custom (in that case, invert it)
					$sort_order = ( (bool) $instance['asc_sort_order'] xor 'custom' == $instance['sort_by'] ) ? 'ASC' : 'DESC';
				} else {
				...
				//note if contextual, this is overiden later
				if($instance['sort_by']=='custom'){
					$sort_by = 'meta_value_num date';
					$meta_key = '_ht_article_order_'.$instance['category'];
				}   

				//note if Category is set to all, it's not possible to use the Sort by Custom  
				...
				if( $instance['contextual'] && is_singular( 'ht_kb' ) ){
					...
					//a possible use case if set to false is to leverage the ht-kb-articles-widget__current-post to style the
					//current article item in the nav
					if( apply_filters( 'ht_kb_articles_widget_exclude_self', true ) ){
						$args['post__not_in'] = array( $post->ID );
					}

					//if sort by custom and we have at least on category, assign meta key as order of first category
					//LIMITATION: This will only order by the first category the article is assigned.
					if( $instance['sort_by']=='custom' && count( $object_terms ) > 0 ){
						$args['sort_by'] = 'meta_value_num date';
						//only the first category will be polled for custom order
						$args['meta_key'] = '_ht_article_order_'.$object_terms[0];
					}
				}
				...
				if( $instance['contextual'] && is_tax( 'ht_kb_category' ) ){
					...
					//if sort by custom, override the current 
					if( $instance['sort_by']=='custom' ){
						$args['sort_by'] = 'meta_value_num date';
						$args['meta_key'] = '_ht_article_order_'.get_queried_object_id();
					} 
				}

				//apply category tax query if required
