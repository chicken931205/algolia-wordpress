...
if( !class_exists( 'HKB_Analytics_Module' )) {
  ...
  class HKB_Analytics_Module {
    ...
    function __construct() {
        ...
        //display notice if there is something blocking the display of (this will be removed by the ht-analytics script)
        add_action( 'admin_notices', array( $this,  'hkba_deprecated_warning' ), 90 );

        //access check filter, plugins at 20-30, themes at 30-40, overrides 50+
        //add_filter( 'ht_analytics_functions', array( $this,  'ht_hkba_check_analytics_functions_access' ), 20 );
    ...
    function ht_hkba_register_menu_page() {
        ...
        if(apply_filters('hkb_analytics_supported', true)){
            //add analytics menu page (now deprecated, will not display, but accessible. will be removed in future versions)
            add_submenu_page( 'edit.php?post_type=ht_kb', 
               __('Heroic Knowledge Base Analytics', 'ht-knowledge-base'), 
                __('Analytics Legacy', 'ht-knowledge-base'), 
            ...
            remove_submenu_page( 'edit.php?post_type=ht_kb', 'hkb-analytics' );
        }
    }
    ...
    */
    ...
    /**
    * Admin warning, as this functionality is deprecated in favour of analytics-reporting modul
    */
    function hkba_deprecated_warning() {
        if(is_admin() && function_exists('get_current_screen')){    
                $screen = get_current_screen();
                if( $screen && 'ht_kb_page_hkb-analytics'===$screen->base && apply_filters( 'hkba_deprecated_warning_notice', true ) && defined( 'HT_KB_ANALYTICS_REPORTING_PAGE_SLUG' ) ):            
                ?>
                    <div id="hkba-deprecated-warning" class="notice notice-warning">
                        <p><?php printf( __( 'This page outdated, please use the new <a href="%s">Analytics and Reporting page</a>, it will be removed in future versions of Heroic Knoweledge Base', 'ht-knowledge-base'), admin_url(  'edit.php?post_type=ht_kb&page=' . HT_KB_ANALYTICS_REPORTING_PAGE_SLUG ) ); ?></p>
                    </div>
                <?php
                endif; 
        } 
    ...
    function hkba_admin_scripts() {
        ...
        if(isset($_GET['page']) && 'hkb-analytics' ===  $_GET['page']) {
            ...
            wp_enqueue_script( 'ht-analytics', plugins_url($ht_analytics_js_src, __FILE__), array('jquery', 'data-tables-script', 'jquery-ui-core', 'jquery-ui-datepicker', 'jquery-ui-tooltip', 'backbone', 'underscore'), HT_KB_VERSION_NUMBER, true );
            ...
            wp_localize_script( 'ht-analytics', 
                                'hkbAnalyticsChart', 
                                array(  'notEnoughResultsString' => __( 'Not enough data to report on', 'ht-knowledge-base'), 
    ...
    /**
     * Check for analytics page access
     * @deprecated
     * functionality moved to standalone plugin analytics pro plugin, packaged with the licensed plugins
     */
    function ht_hkba_check_analytics_functions_access($allow){
        ...
        //requires license active, hook later to ht_analytics_functions to override
        $ht_kb_license_status = get_option('ht_kb_license_status');
        $allow = ('valid'==$ht_kb_license_status) ? $allow : false;
