...
if (!class_exists('HT_Knowledge_Base_Site_Health')) {
			...
			$debug_info['ht-kb'] = array(
				...
				'fields'   => array(
					...
					'license_status' => array(
						...
						'value'   =>  apply_filters( 'ht_kb_site_health_license_status',  get_option( 'ht_kb_license_status', 'empty' )  ),
						'private' => false, //this can be copied
					),					
					'connection' => array(
					...
					*/
					...
					'ht_kb_theme_integration' => array(
						'label'    => __( 'KB Theme Integration', 'ht-knowledge-base' ),
						'value'   => $this->ht_get_theme_integration(),
						'private' => false, //this can be copied
					),
	...
	*/
	...
	/**
	* Get theme integration
	*/
	public function ht_get_theme_integration(){
		$theme_folder = locate_template( 'hkb-templates' . '/' );
		if($theme_folder){
			$path_info = array_filter( explode( '/', $theme_folder ) );
			$theme_folder = sprintf( '%s in %s', array_pop( $path_info ), array_pop( $path_info ) );
		}
		return ( $theme_folder ) ? $theme_folder : __( 'None', 'ht-knowledge-base' );
	}
