    ...
    class HT_Knowledge_Base_Edit_Columns {
        ...
        */
        ...
        function data_kb_custom_data_column( $column, $post_id ) {
            switch ( $column ) {
                ...
                case 'reading_time':
                    ...
                    $reading_time_title = sprintf( _n( '%s minute', '%s minutes', $reading_time_raw, 'ht-knowledge-base' ), $reading_time_raw );
                        ?>
                        <div class="hkb-meta">
                            <div class="hkb-meta__reading_time" title="<?php echo esc_attr( $reading_time_title );  ?>">
                                <?php echo esc_attr( $reading_time_title );  ?>
                            </div>
                        </div>
                        <?php 
                    break;
        ...
        function add_kb_custom_data_column( $columns ) { 
            ...
            if(apply_filters('ht_kb_display_reading_time_column', true)){
                $column_name = __('Reading Time', 'ht-knowledge-base');
                $column_meta = array( 'reading_time' => $column_name );
                $columns = array_slice( $columns, 0, 8, true ) + $column_meta + array_slice( $columns, 8, NULL, true );
            }
        ...
        function kb_custom_data_orderby( $query ) {
            ...
            if( 'article_views' == $orderby && defined('HT_KB_POST_VIEW_COUNT_KEY') ) {
                $query->set( 'meta_key', HT_KB_POST_VIEW_COUNT_KEY );
                $query->set( 'orderby','meta_value_num' );
            }

            if( 'reading_time' == $orderby && defined('HT_KB_POST_READING_TIME_KEY') ) {
                $query->set( 'meta_key', HT_KB_POST_READING_TIME_KEY );
                $query->set( 'orderby','meta_value_num');
            }
            ...
            if( 'article_rating' == $orderby && defined('HT_USEFULNESS_KEY') ) {
