...
*	Version: 3.10.4
*   Build: 3933
*   Build Date: 2023-09-07 4:28:00PM
...
if( !class_exists( 'HT_Knowledge_Base' ) ){
	...
	if(!defined('HT_KB_VERSION_NUMBER')){
		define('HT_KB_VERSION_NUMBER', '3.10.4');
	}

	if ( ! defined( 'HT_KB_BUILD_NUMBER' ) ) {
		define( 'HT_KB_BUILD_NUMBER', 3933 );
	}

	if ( ! defined( 'HT_KB_DISTRIBUTION' ) ) {
		//standalone or packaged
		define( 'HT_KB_DISTRIBUTION', 'standalone' );
	}
	...
	//run setup assistant recommended notice transient
	if(!defined('HT_KB_RUN_SETUP_ASSISTANT_NOTICE_TRANSIENT')){
		define('HT_KB_RUN_SETUP_ASSISTANT_NOTICE_TRANSIENT', '_ht_kb_run_setup_assistant_notice');
	}
	...
	class HT_Knowledge_Base {
		...
		function __construct(){
			...
			//advise user to re-run setup assistant after upgrade
			add_filter( 'ht_kb_dismissable_notices', array( $this, 'ht_kb_register_ht_kb_maybe_run_setup_assistant_notice' ) );
			add_action( 'admin_notices', array( $this, 'ht_kb_maybe_run_setup_assistant_notice' ), 30 );
			//save post
			...
			//sample importer
			include_once('php/ht-knowledge-base-sample-importer.php');
			//updater
			...
			include_once('analytics-reporting/ht-knowledge-base-analytics-reporting.php');
			//analytics module (deprecated, to be removed in future revisions)
			...
			//read time
			include_once('php/ht-knowledge-base-reading-time.php');

			//meta display
			include_once('php/ht-knowledge-base-article-meta-display.php');

			//dismiss notices functionality
			include_once('php/ht-knowledge-base-dismiss-notices.php');

			//reports module
			include_once('php/ht-knowledge-base-reports.php');
			include_once('php/ht-knowledge-base-reports-settings.php');
			include_once('php/ht-knowledge-base-report-helpers.php');
		...
		*/
		function register_ht_knowledge_base_cpt(){
			...
			//@since 3.4 capability type filter 
			$capability_type = apply_filters( 'ht_kb_cpt_capability_type', 'post' );
			//@since 3.4 map meta cap functionality
			$map_meta_cap = apply_filters( 'ht_kb_cpt_map_meta_cap', null );
			...
			$args = apply_filters( 'ht_kb_cpt_args', $args );
		...
		*/
		function register_ht_knowledge_base_category_taxonomy(){
			...
			$capabilities = apply_filters( 'ht_kb_category_capabilities', array(
								//current implementation is default WordPress behaviour
								'manage_terms'  =>   'manage_categories', //'manage_'.'ht_kb_category',
								'edit_terms'    =>   'manage_categories', //'edit_'.'ht_kb_category',
								'delete_terms'  =>   'manage_categories', //'delete'.'ht_kb_category',
								'assign_terms'  =>   'edit_posts', //'assign_'.'ht_kb_category',
							) );
		...
		*/
		function register_ht_knowledge_base_tag_taxonomy()  {
			...
			$capabilities = apply_filters( 'ht_kb_tag_capabilities', array(
								//current implementation is default WordPress behaviour
								'manage_terms'  =>   'manage_categories', //'manage_'.'ht_kb_tag',
								'edit_terms'    =>   'manage_categories', //'edit_'.'ht_kb_tag',
								'delete_terms'  =>   'manage_categories', //'delete'.'ht_kb_tag',
								'assign_terms'  =>   'edit_posts', //'assign_'.'ht_kb_tag',
							) );
		...
		*/
		function ht_kb_database_old_version_check() {
			...
			if( version_compare( HT_KB_VERSION_NUMBER, $db_version ) == 1  ){
				//add a notice to get user to refresh to complete db update
				add_action('admin_notices', array($this, 'ht_kb_database_old_version_check_notice'));

				//3.7.0 Add transient advising user to visit the setup assistant to complete upgrade (no expiry)
				set_transient( HT_KB_RUN_SETUP_ASSISTANT_NOTICE_TRANSIENT, 1 );

				//remove the dismissed option to ensure notices are displayed
				delete_option( HT_KB_DISMISSABLE_NOTICE_OPTION_PREFIX . 'ht-kb-notice-maybe-run-setup-assistant' );
			}
		...
		*/
		...
		/**
		 * Register the ht-kb-notice-maybe-run-setup-assistant as a valid dimissable notice
		 */
		function ht_kb_register_ht_kb_maybe_run_setup_assistant_notice( $notices ) {
			$notices[] = 'ht-kb-notice-maybe-run-setup-assistant';
			return $notices;
		}	

		/**
		* Admin warning to advise user to re-run the setup assistant after upgrade
		*/
		function ht_kb_maybe_run_setup_assistant_notice() {	
			
			//check the transient to show the setup assistant notice
			$show_maybe_run_setup_assistant_notice = get_transient( HT_KB_RUN_SETUP_ASSISTANT_NOTICE_TRANSIENT );
			if( 1 != $show_maybe_run_setup_assistant_notice ){
				return;
			}

			//check if the message has been dismissed
			$run_setup_assistant_notice_dismissed = get_option( HT_KB_DISMISSABLE_NOTICE_OPTION_PREFIX . 'ht-kb-notice-maybe-run-setup-assistant' );
			if ( 1 == $run_setup_assistant_notice_dismissed ) {
				return;
			}

			//add override for display of notice
			if ( !apply_filters( 'ht_kb_maybe_run_setup_assistant_notice_display', true ) ) {
				delete_transient( HT_KB_RUN_SETUP_ASSISTANT_NOTICE_TRANSIENT );
                return;
            }

			//enqueue ajax dismissable library here
			do_action( 'ht_kb_enqueue_dismiss_notices' );
		?>
			<div data-dismissible="ht-kb-notice-maybe-run-setup-assistant" class="notice notice-info ht-kb-ajax-notice-dismiss is-dismissible">
				<p><?php printf( __('Heroic KB has been upgraded. Please run the <a href="%s">Heroic KB Setup Assistant</a> to ensure packaged plugins are updated.', 'ht-knowledge-base'), admin_url( sprintf( 'edit.php?post_type=%s&page=%s', 'ht_kb', HT_KB_WELCOME_SETUP_PAGE ) ) ) ; ?></p>
			</div>
		<?php
		}
		...
		*/
		function ht_kb_no_articles_warning() {
			...
			//disable this warning with show_ht_kb_no_articles_warning filter
			if( !apply_filters( 'show_ht_kb_no_articles_warning', true ) ){
				return;
			}
			...
			?>
				<div class="notice notice-warning">
					<p><?php printf( 
								__('It looks like you have no Knowledge Base articles, you can <a href="%s">add new article now</a>, or install the demo content from the <a href="%s">setup assistant</a>.', 'ht-knowledge-base'),
								admin_url( 'post-new.php?post_type=ht_kb' ),
								admin_url( apply_filters( 'ht_kb_getting_started_page_link', 'index.php?page=ht-kb-welcome' ) )
							);
					 ?></p>
				</div>
			<?php
		...
		*/
		...
		function ht_kb_plugin_row_meta_links( $input, $file ) {
			...
			if( current_theme_supports('ht-kb-theme-managed-updates') &&
				...
				$links[] = '<a href="' . admin_url( apply_filters('ht_kb_getting_started_page_link', 'index.php?page=ht-kb-welcome' ) ) . '">' . esc_html__( 'Setup Assistant', 'ht-knowledge-base' ) . '</a>';	
			}			
		 ...
		 * 
		 * @since 3.4 The `$args` parameter was added.
		 * 
		 * @param (String) $slug Slug
		 * @param (String) $name The filename
		 * @param (Array) $args Additional arguments to be passed to the template (since 3.4)
		 */
		function hkb_get_template_part( $slug, $name = null, $args = array() ) {
			//appends the slug and name parameters to the args for later use
			$args['ht_kb_get_template_part_slug'] = $slug;
			$args['ht_kb_get_template_part_name'] = $name;

			do_action( 'before_hkb_get_template_part_' . $slug );
			$template = hkb_locate_template($slug, $name);
			//args accepted as of WP5.5
		...
		*/
		static function knowledgebase_taxmeta_update_database(){
			...
			//catch errors with get_terms, don't upgrade term meta
			if( ( empty( $taxonomy_terms ) || is_a( $taxonomy_terms, 'WP_Error' ) ) && apply_filters( 'ht_kb_return_on_get_terms_error', true ) ){ 
				update_option( 'hkb_database_terms_db_version', HT_KB_VERSION_NUMBER );
				return;
			}
...
if( class_exists( 'HT_Knowledge_Base' ) ){
	...
	function hkb_get_template_part($slug, $name = null, $args = array() ){
		global $ht_knowledge_base_init;
		return $ht_knowledge_base_init->hkb_get_template_part($slug, $name, $args);
	}
